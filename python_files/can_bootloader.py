...
class BootloaderApp(ttk.Window):
    def __init__(self):
        ...
        icon_path = r"C:\Users\PandurangaKarapothul\Downloads\favicon.ico"
    ...
    def create_widgets(self):
        ...
        firmware_frame = ttk.LabelFrame(main_frame, text="Firmware Update" )
        firmware_frame.pack(fill='x', padx=5, pady=5)
        self.create_firmware_section(firmware_frame)
        
        details_frame = ttk.LabelFrame(main_frame, text="Application Details" )
    ...
    def create_device_section(self, frame):
        ...
        self.baud_var = ttk.StringVar(value="500000")
        baud_menu = ttk.Combobox(row1_frame, textvariable=self.baud_var, values=["500000", "250000", "115200", "9600"], state='readonly', width=10, bootstyle=INFO)
        ...
        ttk.Label(self.can_frame, text="CAN Interface:", bootstyle=DARK, font=("Segoe UI", 10, "bold")).pack(side=LEFT, padx=(0, 10))
    ...
    def on_interface_changed(self, event=None):
        ...
        if interface == "UART":
            ...
            # Update baudrate values for UART
            self.baud_var.set("115200")  # Default for UART
            baud_menu = self.baud_menu if hasattr(self, 'baud_menu') else None
        elif interface == "CAN":
            self.uart_frame.pack_forget()
            self.can_frame.pack(fill='x')
            # Update baudrate values for CAN
            self.baud_var.set("500000")  # Default for CAN
            # Update baudrate options for CAN
            if hasattr(self, 'baud_menu'):
                self.baud_menu.configure(values=["500000", "250000", "125000", "100000"])
    ...
    def create_firmware_section(self, frame):
        ...
        fw_entry = ttk.Entry(frame, textvariable=self.fw_path, width=80, bootstyle=PRIMARY)
    ...
    def connect_uart(self):
        ...
        # Get baudrate from GUI and convert to integer
        baudrate = int(self.baud_var.get())
        self.log_message(f"Connecting to UART at {baudrate} baud")
        
        self.ser = serial.Serial(com_port, baudrate=baudrate, timeout=0.1)
    ...
    def connect_can(self):
        ...
        # Get baudrate from GUI and convert to integer
        baudrate = int(self.baud_var.get())
        self.log_message(f"Connecting to CAN at {baudrate} bps")
        
        if can_interface == "pcan":
            self.bus = can.interface.Bus(
                interface='pcan',
                channel=f'PCAN_USBBUS{channel}',
                bitrate=baudrate  # Use baudrate from GUI
            )
        else:
    ...
    def process_received_data(self, data, buffer):
        """
        Process raw bytes received from the device.
        Commands are single bytes: 0xAA, 0xAD, 0xAB, 0xAC.
        All other bytes are treated as binary data (e.g., version info).
        """
        if not data:
            return buffer

        # Check each byte for a known command
        for byte in data:
            if byte in (0xAA, 0xAD, 0xAB, 0xAC):
                self._handle_command(byte)
            else:
                # Pass non‑command bytes to the binary data handler
                self.handle_binary_data(bytes([byte]))
        return buffer   # buffer is no longer used for line splitting, kept for compatibility

    def _handle_command(self, cmd_byte):
        """Handle a single‑byte command received from the MCU."""
        global app_status

        if cmd_byte == 0xAA:          # OK
            self.log_message("Received OK (0xAA) from MCU - starting firmware transfer")
            self.ok_event.set()
            if self.pending_fwfile and not self.sending:
                self.start_firmware_transfer()

        elif cmd_byte == 0xAD:        # FLASH
            self.log_message("Received FLASH (0xAD) - flashing successful")
            app_status = 1
            self.sending = False
            self.pending_fwfile = None
            self.after(0, lambda: messagebox.showinfo("Success", "Firmware flashing completed successfully!"))

        elif cmd_byte == 0xAB:        # ERROR
            self.log_message("Received ERROR (0xAB) - flashing failed", "ERROR")
            app_status = 0
            self.sending = False
            self.pending_fwfile = None
        ...
        elif cmd_byte == 0xAC:        # WRONG
            self.log_message("Received WRONG (0xAC) - Application linker settings are incorrect", "WRONG")
            app_status = 0
            self.sending = False
            self.pending_fwfile = None
            self.after(0, lambda: messagebox.showerror("Wrong linker settings", "Application linker file settings are wrong. Correct the linker settings and send again."))
            self.fw_status.config(text="[Failed - Retry]", bootstyle="danger")
...
if __name__ == "__main__":
    ...
    """
    Commands:
            Uart Connect = 0x31, 49
            CAN Connect = 0x32, 50
            Load File = 0x33, 51
            Cancel Button = 0x34,52
            Update Device = 0x36, 54
